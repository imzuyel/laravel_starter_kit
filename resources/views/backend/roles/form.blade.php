      ...
      <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0 p-0">
          ...
          <li class="breadcrumb-item active"
            aria-current="page">{{ isset($role->id) ? 'Update role' : 'Create role' }}</li>
        </ol>
      </nav>
    </div>
  </div>
          ...
          <div class="d-flex align-items-center">
            ...
            <div class="ml-auto">
              <a class="btn btn-primary"
                ...
                data-toggle="tooltip"
                title="Back to all roles &#9194;"><i class="bx bx-rewind"></i>Back</a>
            </div>
          </div>
        </div>
        <div class="card-body">
          <form action="{{ isset($role->id) ? route('app.roles.update', $role->id) : route('app.roles.store') }}"
            method="post">
            @csrf
            @isset($role->id)
              @method('PUT')
            ...
            <div class="form-body">
              ...
              <div class="form-group row">
                <label class=" col-form-label text-center text-uppercase mx-auto"
                  style="border-bottom: 1px solid rgb(47, 129, 252);min-width: 35%"><b>Manage
                    permission</b></label>
              </div>
              <br>
              <p class=" text-center">
                ...
                <div class="col-sm-12 mb-3 ml-4">
                  <div class="custom-control custom-checkbox ">
                    <input type="checkbox"
                      ...
                      name="select-all"
                      id="select-all"
                      @if (isset($role->id)) {{ $permission == $role->permissions()->count() ? 'checked' : '' }} @endif>

                    <label class="custom-control-label"
                      for="select-all">All permissions</label>
                  </div>
                </div>
              </div>
              @forelse ($modules->chunk(2)->reverse() as $chunks)
                <div class="row">
